...
import { IsString, IsNotEmpty, IsNumber, IsOptional, validateSync, Min, Max, MinLength } from 'class-validator';

class EnvironmentVariables {
  @IsString()
  @IsNotEmpty({ message: 'JWT_SECRET is required. Generate one with: openssl rand -base64 32' })
  @MinLength(20, { message: 'JWT_SECRET must be at least 20 characters for production' })
  JWT_SECRET!: string;

  @IsString()
  @IsNotEmpty({ message: 'JWT_REFRESH_SECRET is required. Generate one with: openssl rand -base64 32' })
  @MinLength(20, { message: 'JWT_REFRESH_SECRET must be at least 20 characters for production' })
