...
export class AuthService {
  ...
  async googleLogin(profile: GoogleProfile) {
    ...
    await this.userRepo.update(user.id, {
      oauthCode,
      oauthCodeExpires: new Date(Date.now() + 300000),
    });
  ...
  async exchangeOauthCode(code: string) {
    ...
    const user = await this.userRepo.findOne({
      where: {
        oauthCode: code,
        oauthCodeExpires: MoreThan(new Date()),
      },
    });
    ...
    await this.userRepo.update(user.id, {
      oauthCode: null as unknown as string,
      oauthCodeExpires: null as unknown as Date,
    });
  ...
  async logout(userId: string) {
    await this.userRepo.update(userId, { refreshToken: null as unknown as string });
